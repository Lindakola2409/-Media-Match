...
def compute_chroma(audio_path):
    """Computes the chroma features of the audio file and returns them as a 2D array."""
    y, sr = librosa.load(audio_path, sr=None)
...
def find_exact_frame(matched_video, query_chroma, hop_length=512, win_length=2048):
    ...
    y, sr = librosa.load(audio_path, sr=None)
    chroma = librosa.feature.chroma_stft(y=y, sr=sr)
    num_frames = chroma.shape[1] - query_chroma.shape[1] + 1
    min_distance = float('inf')
    best_frame_index = 0
    for i in range(num_frames):
        current_frame_chroma = chroma[:, i:i + query_chroma.shape[1]]
        distance = np.linalg.norm(query_chroma - current_frame_chroma)
        if distance < min_distance:
            min_distance = distance
            best_frame_index = i
...
if __name__ == "__main__":
    ...
    frame_time = find_exact_frame(matched_video, query_chroma)
