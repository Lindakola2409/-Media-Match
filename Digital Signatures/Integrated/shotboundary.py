...
def shotDetect(videopath):
    ...
    data = {}
    data[0] = spot.tolist()
    # with open("./Digital Signatures/Integrated/shot_data.json", "w") as file:
    #     json.dump(data, file)

    if len(data[0]) > 2: # At least 2 shot boundaries
        with open("./Digital Signatures/Integrated/shot_database.json", "r") as file:
        ...
        for key in database:
            ...
            for i in range(len(video_shot) - len(query_shot) + 1):
                if video_shot[i:i+len(query_shot)] == query_shot:
                    ...
                    if(detected.get(key) == None):
                        detected[key] = [frameno]
                    else:
                        detected[key].append(frameno)
        return detected
    else:
        return {}